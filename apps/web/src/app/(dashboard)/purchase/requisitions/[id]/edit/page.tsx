...
import { ArrowLeft, Trash2, Plus, Loader2, Package, Layers, AlertTriangle } from 'lucide-react';
import { usePurchaseRequisitionDetails, useUpdatePurchaseRequisition } from '@/hooks/use-purchase';
...
export default function EditRequisitionPage() {
  ...
  const { data: requisition, isLoading, error } = usePurchaseRequisitionDetails(id);
  const mutation = useUpdatePurchaseRequisition();
  ...
  React.useEffect(() => {
    if (requisition) {
      reset({
        ...
        warehouseName: requisition.warehouseName,
      ...
      <PageContainer>
        <div className="text-center py-16 bg-card border rounded-2xl">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            The requested purchase requisition does not exist.
          </p>
    ...
    return (
      <PageContainer>
        <div className="text-center py-16 bg-card border rounded-2xl">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Only requisitions in DRAFT status can be modified. This requisition has already been
            submitted.
          </p>
          <Link href={`/purchase/requisitions/${id}`} className="inline-block mt-4">
            <Button size="sm">View Requisition Details</Button>
          </Link>
        </div>
      </PageContainer>
    );
  ...
  return (
    <PageContainer>
      <div className="mb-4">
        <Link href={`/purchase/requisitions/${id}`}>
      ...
      <PageHeader
        ...
        description={`Modify properties for requisition: ${requisition.title}`}
      />

            ...
            <CardContent className="p-4">
              <form onSubmit={handleCreate} className="space-y-4 text-xs sm:text-sm">
                <div className="grid gap-1">
                  <label className="text-muted-foreground font-semibold">Group Name</label>
                  <Input
                    type="text"
                ...
                <div className="grid gap-1.5 text-left">
                  <label className="text-muted-foreground font-semibold">Account Type</label>
                  <select
                    value={gpType}
        ...
        <div className="md:col-span-2">
          <Card className="bg-cardard border-border text-foreground flex flex-col h-full overflow-hidden">
            ...
            <CardContent className="flex-1 p-0 overflow-y-auto max-h-[420px] custom-scrollbar text-xs">
              {isLoading ? (
                <div className="text-center py-10 text-muted-foreground">Querying groups...</div>
              ) : groups.length > 0 ? (
                <table className="w-full text-xs text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-border text-muted-foreground">
                    {groups.map((gp) => (
                      <tr key={gp.id} className="hover:bg-accent/10">
                        <td className="py-3 px-4 font-bold text-foreground">{gp.name}</td>

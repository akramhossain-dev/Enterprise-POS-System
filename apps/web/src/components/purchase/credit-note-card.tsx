    ...
    <Card className="border border-border bg-card shadow-sm hover:shadow-md transition-shadow rounded-xl overflow-hidden text-sm">
      <CardHeader className="bg-muted/30 border-b p-4 flex flex-row items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono font-bold text-foreground">{creditNote.creditNoteNumber}</span>
        </div>
        <div className="flex items-center gap-2">
          <Badge className={statusStyles[creditNote.status] || 'bg-muted'}>
            {creditNote.status}
          </Badge>
          {onPrint && (
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-mono font-semibold text-primary">
            {creditNote.referenceReturnNumber}
          </span>
        </div>
        <div className="flex justify-between">
        ...
        <div className="border-t border-dashed border-border/80 my-2 pt-2.5 flex justify-between items-baseline">
          <span className="font-semibold text-foreground">Credited Amount</span>
          <span className="text-lg font-bold font-mono text-emerald-500">
            ${Number(creditNote.creditAmount).toFixed(2)}
          </span>
        </div>
      </CardContent>
    </Card>
